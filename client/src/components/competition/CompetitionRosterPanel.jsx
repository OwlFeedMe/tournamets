...
function buildCategoryTone(label, theme) {
  ...
  return {
    glow: hexToRgba(theme.accent || '#00C2A8', 0.18),
    text: theme.accent || '#00C2A8',
    pillBg: hexToRgba(theme.accent || '#00C2A8', 0.12),
    pillBorder: hexToRgba(theme.accent || '#00C2A8', 0.24),
  }
}
      ...
      <div style={{ padding: compact ? '11px 11px 12px' : '14px 14px 16px' }}>
        <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: compact ? 20 : 26, lineHeight: 1.02, letterSpacing: '0.04em', marginBottom: compact ? 6 : 8, overflowWrap: 'anywhere' }}>
          {item.type === 'participant' ? (
            <AthleteNameLink username={item.username}>{displayName}</AthleteNameLink>
          ) : (
            displayName
          )}
        </div>
      ...
      >
        ...
        <div style={{ marginTop: -62 }}>
          ...
          <div style={{ padding: '20px 24px 28px', display: 'grid', gap: 18 }}>
            <div>
              <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 40, lineHeight: 1, letterSpacing: '0.03em', color: '#F5F7FA' }}>
                {item.type === 'participant' ? (
                  <AthleteNameLink username={item.username}>{displayName}</AthleteNameLink>
                ) : (
                  displayName
                  ...
                  {item.members.map((member, index) => (
                    <div key={member.id || `${item.id}-member-${index}`} style={{ display: 'flex', alignItems: 'center', gap: 10, borderRadius: 10, border: '1px solid #252A33', background: 'rgba(255,255,255,0.03)', padding: '10px 12px' }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ color: '#F5F7FA', fontWeight: 700, fontSize: 13 }}>
                          <AthleteNameLink username={member.username}>
...
export function CompetitionRosterPanel({
  competitionId,
  competition: suppliedCompetition = null,
  initialRosterPayload = null,
  embedded = false,
}) {
  const [competitionPayload, setCompetitionPayload] = useState(suppliedCompetition ? { competition: suppliedCompetition } : null)
  const [rosterPayload, setRosterPayload] = useState(initialRosterPayload)
  const [countries, setCountries] = useState([])
  const [loading, setLoading] = useState(!initialRosterPayload)
  ...
  useEffect(() => {
    if (!initialRosterPayload) return
    setRosterPayload(initialRosterPayload)
    setLoading(false)
  }, [initialRosterPayload])

  useEffect(() => {
    let active = true
    const shouldReuseInitialRoster = !!initialRosterPayload
    if (shouldReuseInitialRoster && suppliedCompetition) {
      setCompetitionPayload({ competition: suppliedCompetition })
      setError('')
      setLoading(false)
      return () => {
        active = false
    ...
    setError('')
    Promise.all([
      suppliedCompetition ? Promise.resolve({ data: { competition: suppliedCompetition } }) : api.get(`/competitions/${competitionId}/public`),
  ...
  }, [competitionId, initialRosterPayload, suppliedCompetition])
  ...
  const competition = competitionPayload?.competition || suppliedCompetition || null
  ...
  const shellStyle = embedded
    ? { display: 'grid', gap: 18 }
    : { minHeight: '100vh', background: '#0D0F12', color: '#F5F7FA' }
  const innerStyle = embedded
    ? {}
    : { maxWidth: COMPETITION_PAGE_MAX_WIDTH, margin: '0 auto', padding: '32px 24px 72px' }
        ...
        {loading ? (
          ...
          <>
            ...
            >
              <div style={{
                ...
                <div style={{
                  ...
                  gap: 16,
                  flexWrap: 'wrap',
                  padding: embedded ? 0 : (isMobile ? '18px 14px 12px' : '18px 24px 12px'),
                  borderBottom: embedded ? 'none' : '1px solid #252A33',
                }}>
                  <div style={{ display: 'grid', gap: 8 }}>
                    <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 11, fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#FF6B00' }}>
                      Inscritos confirmados
                    </div>
                    <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: embedded ? (isMobile ? 30 : 42) : 30, lineHeight: 1, letterSpacing: '0.04em', color: '#F5F7FA' }}>
                      {competition?.nombre || 'FinalRep'}
                    </div>
                    <div style={{ maxWidth: 720, color: '#AAB2C0', fontSize: 14, lineHeight: 1.65 }}>
                      Filtra por categoria, busca atletas o revisa equipos sin salir del contexto de la competencia.
                    </div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  gap: 10,
                  padding: embedded ? 0 : (isMobile ? '0 14px 14px' : '0 24px 18px'),
                }}>
                  {[
                    ['Inscritos', stats.athletes, '#F5F7FA'],
                    ['Paises', stats.countries, '#F5F7FA'],
                    ['Categorias', stats.categories, '#00C2A8'],
                  ].map(([label, value, color]) => (
                    <div key={label} style={{ borderRadius: 14, border: '1px solid #252A33', background: 'rgba(13,15,18,0.56)', padding: isMobile ? 14 : 16 }}>
                      <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 34, lineHeight: 1, color }}>{value}</div>
                      <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#6B7280', marginTop: 4 }}>
                        {label}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: isMobile ? '1fr' : 'minmax(220px, auto) auto minmax(240px, 1fr)',
                  gap: 12,
                  alignItems: 'center',
                  padding: embedded ? 0 : (isMobile ? '0 14px 18px' : '0 24px 20px'),
                }}>
                  <CatDropdown
                    ...
                    <div style={{
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: mobileView === 'cards'
                            ? 'linear-gradient(135deg, rgba(255,107,0,0.24) 0%, rgba(228,94,0,0.14) 100%)'
                            : 'transparent',
                          ...
                          justifyContent: 'center',
                          cursor: 'pointer',
                          boxShadow: mobileView === 'cards' ? 'inset 0 0 0 1px rgba(255,107,0,0.28)' : 'none',
                        }}
                      >
                        <LayoutGrid size={16} />
                      </button>
                      <button
                        ...
                        style={{
                          ...
                          justifyContent: 'center',
                          cursor: 'pointer',
                          boxShadow: mobileView === 'list' ? 'inset 0 0 0 1px rgba(255,107,0,0.28)' : 'none',
                        }}
                      >
                        <List size={16} />
                      </button>
                  ...
                  <label style={{ position: 'relative', minWidth: isMobile ? '100%' : 240, width: '100%' }}>
                    ...
                    <input
                      ...
                      style={{ width: '100%', minHeight: 42, borderRadius: 10, border: '1px solid #252A33', background: 'rgba(255,255,255,0.04)', color: '#F5F7FA', fontSize: 13, padding: '0 14px 0 36px', outline: 'none' }}
                    />
                  </label>
                </div>
              </div>
            ...
            {!rosterPayload?.enabled ? (
              <div className="fr-cut-card" style={{ padding: 24, background: '#171B21', border: '1px solid #252A33', color: '#AAB2C0', lineHeight: 1.6, marginTop: embedded ? 0 : 18 }}>
                Esta competencia no tiene publicados sus inscritos todavia.
              </div>
            ) : (
              <div style={{ display: 'grid', gap: 32, marginTop: embedded ? 0 : 18 }}>
                {selectedCategoryKey === 'all' && !searchQuery.trim()
                      ...
                      return (
                        <section key={section.key} style={{ display: 'grid', gap: 20 }}>
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                            <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 24, letterSpacing: '0.05em', color: '#F5F7FA' }}>{section.category_name}</div>
                            <div style={{ fontFamily: '"Barlow Condensed", sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', color: '#00C2A8' }}>
                  ...
                  : (
                    <section style={{ display: 'grid', gap: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                        <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 24, letterSpacing: '0.05em', color: '#F5F7FA' }}>
                          {searchQuery.trim()
                            ? 'Resultados'
                      ...
                      {canLoadMore ? (
                        <button
                          ...
                          style={{ justifySelf: 'flex-start', minHeight: 42, borderRadius: 999, border: '1px solid rgba(255,107,0,0.24)', background: 'rgba(255,107,0,0.10)', color: '#F5F7FA', padding: '0 18px', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}
                        >
                          Ver mas
                        </button>
                      ) : null}
...
export default CompetitionRosterPanel
