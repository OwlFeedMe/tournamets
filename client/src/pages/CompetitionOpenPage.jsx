...
import { openRegistrationState, openSubmissionState } from '../utils/openQualifier'
...
export default function CompetitionOpenPage() {
  ...
  const preregister = event => {
    ...
    act(async () => {
      ...
      setMessage('Preinscripción guardada. Ya apareces en la competencia; puedes pagar el Open más adelante, antes del cierre de entregas.')
    })
  }
  return <main className="fr-open" style={{ maxWidth: 960, margin: '0 auto', minHeight: '70vh' }}>
    ...
    {!session ? <Link className="fr-open-button" to="/login">Crear cuenta o iniciar sesión para preinscribirme</Link> : ((!entry && registration.available) || (preregistered && !closed && !!data.activa) || finalPayment) && <form className="fr-open-card" onSubmit={!entry ? preregister : pay}>
