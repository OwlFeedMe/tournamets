...
function colorForWorkoutId(workoutId, index = 0) {
  const raw = String(workoutId || '')
  const numeric = Number(raw)
  if (Number.isFinite(numeric) && numeric > 0) return workoutAccentPalette[(numeric - 1) % workoutAccentPalette.length]
  const hash = raw.split('').reduce((sum, char) => sum + char.charCodeAt(0), index)
  return workoutAccentPalette[Math.abs(hash) % workoutAccentPalette.length]
}

function buildWorkoutColorMap(phases = []) {
  return [...phases]
    ...
    .reduce((map, phase, index) => {
      map[String(phase.id)] = workoutAccentPalette[index % workoutAccentPalette.length]
      return map
    }, {})
}
...
function PhasesPanel({ bundle, reload, notify }) {
  ...
  const orderedPhases = [...(bundle.phases || [])]
    .filter((phase) => !phase.parent_phase_id)
    .sort((a, b) => Number(a.orden || 0) - Number(b.orden || 0) || Number(a.id || 0) - Number(b.id || 0))
  const workoutColorMap = buildWorkoutColorMap(orderedPhases)
  ...
  return (
    <Panel title="Fases y workouts" subtitle="Crea los eventos deportivos que alimentan resultados y heats." action={<Button tone="primary" onClick={() => { reset(); setModalOpen(true) }}><Plus size={16} />Crear fase</Button>}>
      ...
      {orderedPhases.map((phase, index) => (
        <div key={phase.id} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto auto auto', gap: 10, alignItems: 'center', border: `1px solid ${(workoutColorMap[String(phase.id)] || colorForWorkoutId(phase.id, index))}66`, borderLeft: `5px solid ${workoutColorMap[String(phase.id)] || colorForWorkoutId(phase.id, index)}`, background: `${workoutColorMap[String(phase.id)] || colorForWorkoutId(phase.id, index)}10`, borderRadius: 8, padding: 10 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><strong>{phase.nombre}</strong><span aria-hidden="true" style={{ width: 10, height: 10, borderRadius: 999, background: workoutColorMap[String(phase.id)] || colorForWorkoutId(phase.id, index), boxShadow: `0 0 0 3px ${(workoutColorMap[String(phase.id)] || colorForWorkoutId(phase.id, index))}22` }} /></div>
...
function HeatsPanel({ bundle, reload, notify }) {
  ...
  const workoutColorMap = buildWorkoutColorMap(bundle.phases || [])
  const heatParticipants = (heat) => (heat.participants || heat.assignments || [])
  const heatWorkoutColor = (heat) => workoutColorMap[String(heat?.phase_id || 'sin-fase')] || colorForWorkoutId(heat?.phase_id || 'sin-fase')
  const heatLocations = [
    ...customLocations,
    ...heats
  ...
  const heatDestinations = (heat) => heats
    ...
    .sort((a, b) => {
      ...
      return aStart - bStart
        ...
        || String(a.categoria || 'Todas').localeCompare(String(b.categoria || 'Todas'), 'es', { numeric: true })
  ...
  const heatsByWorkout = heats.reduce((groups, heat) => {
    ...
    if (!workout) {
      workout = { id: workoutId, name: heat.phase_name || `WOD ${heat.phase_id || ''}`, color: workoutColorMap[workoutId] || colorForWorkoutId(workoutId, groups.length), categories: [], heats: 0, athletes: 0 }
      groups.push(workout)
    }
    ...
    <Panel title="Heats y orden de salida" subtitle="Participantes y horarios se ajustan por separado." action={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button onClick={() => setCategoryOrderModal(true)}><Settings2 size={16} />Orden categorias</Button><Button onClick={openTimingConfiguration}><Clock3 size={16} />Configurar tiempos</Button><Button onClick={openBulkSchedule}><Clock3 size={16} />Mover horarios</Button><Button tone="primary" onClick={() => setModalOpen(true)} primaryAction><Zap size={16} />Generar heats</Button></div>}>
            ...
            {scheduleDayHeats.length ? (
              [...scheduleDayHeats]
                ...
                .map((heat) => {
                  ...
                  const workoutTone = heatWorkoutColor(heat)
                  return (
                    <div key={heat.id} className="fr-schedule-card" style={{ border: `1px solid ${locationConflict ? colors.error : workoutTone}88`, borderLeft: `5px solid ${workoutTone}`, background: locationConflict ? 'rgba(239,68,68,0.10)' : `${workoutTone}14`, borderRadius: 8, padding: 12, display: 'grid', gap: 10 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {showingAllSchedule ? <Pill tone={workoutTone}>{heat.phase_name || `WOD ${heat.phase_id || ''}`}</Pill> : null}
                            ...
                            {slotHeats.map((heat) => {
                              ...
                              return (
                                <div
                                  ...
                                  style={{ border: `1px solid ${locationConflict ? colors.error : workoutTone}88`, borderLeft: `5px solid ${workoutTone}`, background: locationConflict ? 'rgba(239,68,68,0.10)' : `${workoutTone}14`, borderRadius: 8, padding: 9, cursor: 'grab', display: 'grid', gap: 6 }}
                                >
                                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                                          <strong style={{ color: colors.text, fontSize: 13 }}>{heat.heat_label || heat.nombre || `Heat ${heat.heat_number}`}</strong>
                                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {heatsByWorkout.map((workout, workoutIndex) => {
          ...
          const workoutTone = workout.color || colorForWorkoutId(workout.id, workoutIndex)
          const workoutStatusTone = workout.status?.color || heatStatusColors.upcoming
          return (
          <section key={workout.id} style={{ border: `1px solid ${colors.border}`, background: colors.top, borderRadius: 8, overflow: 'hidden' }}>
              ...
              <button type="button" onClick={() => setCollapsedWorkouts((prev) => ({ ...prev, [workout.id]: !collapsed }))} style={{ border: 0, background: 'transparent', color: colors.text, padding: 0, display: 'flex', gap: 10, alignItems: 'center', textAlign: 'left', minWidth: 0 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 7 }}>
                    <Pill tone={workoutStatusTone}>{workout.status?.label || 'Proximo'}</Pill>
            ...
            {!collapsed ? <div style={{ display: 'grid', gap: 0, padding: '2px 10px 10px' }}>
              {workout.categories.map((group) => {
                const groupTone = group.status?.color || workoutStatusTone
                return (
                <section key={`${workout.id}-${group.category}`} style={{ borderBottom: `1px solid ${colors.border}`, background: 'transparent', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'grid', gap: 0, padding: '0 0 8px' }}>
                    {group.heats.map((heat) => {
                      ...
                      const workoutTone = heatWorkoutColor(heat)
                      return (
                        <div key={heat.id} style={{ borderTop: `1px solid ${colors.border}`, borderLeft: `4px solid ${locationConflict ? colors.error : workoutTone}`, background: locationConflict ? 'rgba(239,68,68,0.08)' : `${workoutTone}0F`, padding: '10px 0 10px 10px', display: 'grid', gap: 8 }}>
    ...
    <Panel title="Puntuacion" subtitle="Define como las posiciones de cada WOD se convierten en puntos del leaderboard." action={<Button tone="primary" onClick={save} disabled={saving}><Save size={16} />{saving ? 'Aplicando...' : 'Aplicar regla'}</Button>}>
      ...
      <div style={{ border: `1px solid ${colors.border}`, background: colors.top, borderRadius: 8, padding: 12, display: 'grid', gap: 10 }}>
        ...
        {showAdvancedOverrides ? (
          (bundle.phases || []).length ? (bundle.phases || []).map((phase, index) => {
...
function ResultsPanel({ bundle, reload, notify }) {
  ...
  const heatResultRow = (heat, participant) => ({
    ...participant,
    heat_id: heat.id,
    heat_label: heat.heat_label || heat.nombre || `Heat ${heat.heat_number}`,
    heat_name: heat.nombre,
    heat_number: heat.heat_number,
    categoria: heat.categoria || participant.categoria || 'Todas',
  })
  const scoreRows = selectedHeat
    ? heatParticipants(selectedHeat).map((participant) => heatResultRow(selectedHeat, { ...participant, categoria: participant.categoria || activeCategory || 'Todas' }))
    : []
  ...
  const sortResultRows = (items) => [...items].sort((a, b) => {
    const categoryOrder = String(a.categoria || 'Todas').localeCompare(String(b.categoria || 'Todas'))
    if (categoryOrder) return categoryOrder
    const heatA = Number(a.heat_number)
    const heatB = Number(b.heat_number)
    const hasHeatA = Number.isFinite(heatA)
    const hasHeatB = Number.isFinite(heatB)
    if (hasHeatA && hasHeatB && heatA !== heatB) return heatA - heatB
    if (hasHeatA !== hasHeatB) return hasHeatA ? -1 : 1
    const laneA = Number(a.lane_number)
    const laneB = Number(b.lane_number)
    const hasLaneA = Number.isFinite(laneA) && laneA > 0
    const hasLaneB = Number.isFinite(laneB) && laneB > 0
  ...
  const uniqueResultRows = (items) => items.reduce((list, item) => {
    const key = resultEntityKey(item)
    if (!key.includes('undefined') && !list.some((existing) => resultEntityKey(existing) === key)) list.push(item)
    return list
  }, [])
  const phaseResultRows = sortResultRows(uniqueResultRows(
    phaseHeats.length
      ? phaseHeats.flatMap((heat) => heatParticipants(heat).map((participant) => heatResultRow(heat, participant)))
      : allConfirmedRows,
  ))
  const normalizeResultSearch = (value) => String(value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .trim()
  const normalizedResultSearch = normalizeResultSearch(resultSearch)
  const resultSearchMatches = (row) => [
    participantName(row),
    row.heat_label,
    row.heat_name,
    row.categoria,
  ...
  ].some((value) => normalizeResultSearch(value).includes(normalizedResultSearch))
  const visibleRows = normalizedResultSearch
    ? phaseResultRows.filter(resultSearchMatches)
    : rows
  ...
  const markValue = (item) => {
    const key = resultKey(activeResultPhaseId, item)
    if (Object.prototype.hasOwnProperty.call(marks[key] || {}, 'dns') && marks[key].dns === true) return ''
  ...
  const isDnfValue = (item) => {
    const key = resultKey(activeResultPhaseId, item)
    if (Object.prototype.hasOwnProperty.call(marks[key] || {}, 'dns') && marks[key].dns === true) return false
  ...
  const isDnsValue = (item) => {
    const key = resultKey(activeResultPhaseId, item)
    if (Object.prototype.hasOwnProperty.call(marks[key] || {}, 'dns')) return !!marks[key].dns
    return String(existingResultFor(item)?.result_status || '').toLowerCase() === 'dns'
  }
  ...
  const setDnsResult = (item) => {
    const key = resultKey(activeResultPhaseId, item)
    const nextActive = !isDnsValue(item)
    setMarks((prev) => ({
      ...prev,
      [key]: {
        ...(prev[key] || {}),
  ...
  const previewRankMap = (() => {
    ...
    const entities = previewPool.map((item) => {
      ...
      const draftDns = !!draft.dns
      const draftDnf = !!draft.dnf && !draftCapDnf && !draftDns
      ...
      const marca = draftDns ? null : draftDnf ? dnfMark() : parseMarkValue(rawMarca)
  ...
  const saveFastResults = async () => {
    ...
    try {
      ...
      for (const row of changed) {
        ...
        const rowDns = !!rowDraft.dns
        const value = markValue(row)
        if (!rowDns && value === '') continue
        ...
        const rowDnf = !!rowDraft.dnf && !rowCapDnf && !rowDns
        const parsedMark = rowDns ? null : rowDnf ? dnfMark() : rowCapDnf ? timeCapSeconds : parseMarkValue(value)
        if (!rowDns && !rowDnf && parsedMark === null) return notify(isTimePhase ? 'Tiempo invalido. Usa MM:SS o HH:MM:SS' : 'Marca invalida', 'error')
        if (!rowDns && !rowDnf && timeCapSeconds && parsedMark > timeCapSeconds) return notify(`El tiempo no puede superar el cap de ${timeCapLabel}`, 'error')
        const parsedTiebreak = rowDns || !tieBreakActive || rowDnf || tiebreak === '' ? null : parseTiebreakValue(tiebreak)
        if (!rowDns && tieBreakActive && !rowDnf && tiebreak !== '' && parsedTiebreak === null) return notify(isTiebreakTime ? 'Tiebreak invalido. Usa MM:SS o HH:MM:SS' : 'Tiebreak invalido', 'error')
        if (!rowDns && rowCapDnf && extra === '') return notify('Ingresa reps faltantes. Usa 0 si termino justo en el cap.', 'error')
        const parsedExtra = !rowDns && rowCapDnf ? Number(extra) : null
        ...
        if (existing) {
          await api(`/results/${existing.id}`, {
            method: 'PUT',
            body: JSON.stringify({ marca: parsedMark, extra: parsedExtra, tiebreak: tieBreakActive ? parsedTiebreak : null, result_status: rowDns ? 'dns' : 'valid' }),
          })
        } else {
          await api('/results', {
            method: 'POST',
            body: JSON.stringify({
              ...
              result_status: rowDns ? 'dns' : 'valid',
            }),
          })
        }
        saved += 1
  ...
  const saveRowResult = async (row) => {
    ...
    const rowDnf = !!rowDraft.dnf && !rowCapDnf && !rowDns
    const value = markValue(row)
    if (!rowDns && !rowDnf && value === '') return notify('Ingresa una marca, DNF o DNS', 'error')
    const extra = extraValue(row)
    const tiebreak = tieBreakActive ? tiebreakValue(row) : ''
    const existing = existingResultFor(row)
    const parsedMark = rowDns ? null : rowDnf ? dnfMark() : rowCapDnf ? timeCapSeconds : parseMarkValue(value)
  ...
  const isPartDns = (row, part) => {
    const key = partResultKey(part, row)
    if (Object.prototype.hasOwnProperty.call(marks[key] || {}, 'dns')) return !!marks[key].dns
    return String(existingResultForPart(row, part)?.result_status || '').toLowerCase() === 'dns'
  }
  const setPartDnfResult = (row, part) => {
    ...
    setMarks((prev) => ({
      ...
      [key]: {
        ...(prev[key] || {}),
        dnf: !isPartDnf(row, part),
        dns: false,
      },
    }))
  }
  const setPartDnsResult = (row, part) => {
    const key = partResultKey(part, row)
    const nextActive = !isPartDns(row, part)
    setMarks((prev) => ({
      ...prev,
      [key]: {
        ...(prev[key] || {}),
  ...
  const validatePartDraft = (row, part) => {
    ...
    const draft = marks[key] || {}
    const cap = partTimeCap(part)
    const dnsActive = !!draft.dns
    const capActive = !!draft.capDnf
    const dnfActive = !!draft.dnf && !capActive && !dnsActive
    const value = partMarkValue(row, part)
    if (!dnsActive && !dnfActive && value === '') return null
    const parsedMark = dnsActive ? null : dnfActive ? (partLowerIsBetter(part) ? DNF_MARK_HIGH : DNF_MARK_LOW) : capActive ? cap : parsePartMarkValue(part, value)
    if (!dnsActive && !dnfActive && parsedMark === null) throw new Error(partIsTime(part) ? `Tiempo invalido en ${part.score_key || part.nombre}` : `Marca invalida en ${part.score_key || part.nombre}`)
    if (!dnsActive && !dnfActive && cap && parsedMark > cap) throw new Error(`El tiempo de ${part.score_key || part.nombre} no puede superar el cap de ${formatSeconds(cap)}`)
    const extra = partExtraValue(row, part)
    if (!dnsActive && capActive && extra === '') throw new Error(`Ingresa reps faltantes en ${part.score_key || part.nombre}. Usa 0 si termino justo en el cap.`)
    const parsedExtra = !dnsActive && capActive ? Number(extra) : null
    ...
    const tiebreak = partTieActive && !dnfActive && !dnsActive ? partTiebreakValue(row, part) : ''
    const parsedTiebreak = dnsActive || !partTieActive || dnfActive || tiebreak === '' ? null : parsePartTiebreakValue(part, tiebreak)
    if (!dnsActive && partTieActive && !dnfActive && tiebreak !== '' && parsedTiebreak === null) throw new Error(partTieBreakIsTime(part) ? `Tiebreak invalido en ${part.score_key || part.nombre}. Usa MM:SS o HH:MM:SS` : `Tiebreak invalido en ${part.score_key || part.nombre}`)
    return {
      key,
      ...
      payload: {
        ...
        result_status: dnsActive ? 'dns' : 'valid',
      },
    }
  }
  const saveMultiPartChanges = async (rowFilter = null) => {
    ...
    try {
      let saved = 0
      const keysToClear = []
      const targetRows = rowFilter ? phaseResultRows.filter(rowFilter) : rows
        ...
        for (const part of resultParts) {
          ...
          if (item.existing) {
            await api(`/results/${item.existing.id}`, {
              method: 'PUT',
              body: JSON.stringify({ marca: item.payload.marca, extra: item.payload.extra, tiebreak: item.payload.tiebreak, result_status: item.payload.result_status }),
            })
          } else {
        ...
        <div className="fr-results-editor-shell" style={{ border: `1px solid ${colors.border}`, background: colors.top, borderRadius: 8, padding: 12, display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 12, minWidth: 0 }}>
          <div className="fr-form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 10 }}>
            ...
            <Field label="Buscar atleta">
              <input style={inputStyle()} value={resultSearch} onChange={(event) => setResultSearch(event.target.value)} placeholder="Nombre, carril, heat o categoria" />
            </Field>
            <div className="fr-results-progress" style={{ display: 'grid', gap: 7, alignSelf: 'end', width: 'min(100%, 220px)', justifySelf: 'end' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, color: colors.text, fontSize: 12, fontWeight: 900 }}>
                <span>{normalizedResultSearch ? `${visibleRows.length}/${phaseResultRows.length}` : rows.length} atletas</span>
                ...
                return (
                  <div key={resultEntityKey(row)} style={{ display: 'grid', gap: 12, padding: 12, border: `1px solid ${rowDirty ? 'rgba(255,107,0,0.55)' : colors.border}`, borderRadius: 8, background: rowDirty ? 'rgba(255,107,0,0.08)' : colors.top }}>
                    <div className="fr-multipart-result-head" style={{ display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr) auto auto', gap: 10, alignItems: 'center' }}>
                      ...
                      <span className="fr-multipart-result-athlete" style={{ color: colors.text, fontSize: 15, lineHeight: 1.25, fontWeight: 900, minWidth: 0, overflowWrap: 'anywhere' }}>
                        {participantName(row)}
                        {normalizedResultSearch ? <small style={{ display: 'block', color: colors.secondary, fontSize: 11, fontWeight: 800, marginTop: 3 }}>{row.categoria || 'Todas'} · {row.heat_label || row.heat_name || 'Sin heat'}</small> : null}
                      </span>
                    ...
                    <div className="fr-form-grid" style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(resultParts.length, 2)}, minmax(240px, 1fr))`, gap: 10 }}>
                      {resultParts.map((part) => {
                        ...
                        const dnsActive = isPartDns(row, part)
                        const dnfActive = isPartDnf(row, part) && !capActive && !dnsActive
                        const partTieActive = partTieBreakActive(part)
                        return (
                          <div key={part.id} style={{ border: `1px solid ${dirty ? 'rgba(255,107,0,0.55)' : colors.border}`, borderRadius: 8, background: colors.surface, padding: 10, display: 'grid', gap: 9 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: capActive ? 'minmax(0, 1fr) minmax(120px, .7fr) auto auto' : 'minmax(0, 1fr) auto auto', gap: 8, alignItems: 'end' }}>
                              <Field label={isTime ? 'Tiempo' : part?.tipo === 'posicion' ? 'Posicion' : 'Marca'}>
                                <input type={isTime ? 'text' : 'number'} style={inputStyle()} value={dnsActive || dnfActive ? '' : capActive && cap ? formatSeconds(cap) : partMarkValue(row, part)} onWheel={preventNumberInputWheel} disabled={dnsActive || dnfActive || capActive} placeholder={dnsActive ? 'DNS' : dnfActive ? 'DNF' : isTime ? (cap ? formatSeconds(cap) : '12:00') : 'Valor'} onChange={(event) => setPartResultField(row, part, 'marca', isTime ? formatTimeEntryInput(event.target.value) : event.target.value)} />
                              </Field>
                              {capActive ? (
                              ...
                              <Button onClick={() => setPartDnsResult(row, part)} tone={dnsActive ? 'danger' : 'default'}>DNS</Button>
                            </div>
                            {partTieActive ? (
                              <Field label={partTieBreakLabel(part)}>
                                <input type={partTieBreakIsTime(part) ? 'text' : 'number'} step="1" style={inputStyle()} value={dnsActive || dnfActive ? '' : partTiebreakValue(row, part)} onWheel={preventNumberInputWheel} disabled={dnsActive || dnfActive} placeholder={dnsActive ? 'DNS' : partTieBreakIsTime(part) ? '01:23' : 'Opcional'} onChange={(event) => setPartResultField(row, part, 'tiebreak', partTieBreakIsTime(part) ? formatTimeEntryInput(event.target.value) : event.target.value)} />
                              </Field>
                            ) : null}
      ...
      <div className="fr-results-editor-shell" style={{ border: `1px solid ${colors.border}`, background: colors.top, borderRadius: 8, padding: 12, display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 12, minWidth: 0 }}>
        <div className="fr-form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 1fr))', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap' }}>
            <Pill tone={colors.primary}>{normalizedResultSearch ? `${visibleRows.length}/${phaseResultRows.length}` : rows.length} atletas</Pill>
        ...
        <div className="fr-results-table" style={{ border: `1px solid ${colors.border}`, borderRadius: 8, overflow: 'hidden', background: colors.surface }}>
          <div className="fr-results-list" style={{ display: 'grid', gap: 10, maxHeight: 560, overflowY: 'auto', padding: 12 }}>
            {visibleRows.length ? visibleRows.map((row) => {
              ...
              const rowDns = isDnsValue(row)
              const rowDnf = isDnfValue(row) && !capDnf && !rowDns
              ...
              return (
                <div className="fr-result-row" key={key} style={{ display: 'grid', gap: 12, padding: 12, border: `1px solid ${dirty ? 'rgba(255,107,0,0.55)' : colors.border}`, borderRadius: 8, background: dirty ? 'rgba(255,107,0,0.08)' : colors.top }}>
                    ...
                    <div className="fr-result-actions" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                      {editable ? (
                        <>
                          ...
                          <Button onClick={() => setDnsResult(row)} tone={rowDns ? 'danger' : 'default'}>DNS</Button>
                  ...
                  <div className="fr-result-card-grid" style={{ display: 'grid', gridTemplateColumns: resultCardGridColumns, gap: 10, alignItems: 'stretch' }}>
                    <span className="fr-result-heat fr-result-card-field" data-label="Heat" style={{ color: colors.secondary, fontSize: 13, overflowWrap: 'anywhere' }}>{normalizedResultSearch ? `${row.categoria || 'Todas'} · ${row.heat_label || row.heat_name || 'Sin heat'}` : (row.heat_label || selectedHeat?.heat_label || selectedHeat?.nombre || 'Sin heat')}</span>
                    {editable ? (
                      <label className="fr-result-input-wrap">
                        ...
                        <span style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 6 }}>
                          <input className="fr-result-mark" type={isTimePhase ? 'text' : 'number'} style={inputStyle()} value={rowDns || rowDnf ? '' : capDnf ? timeCapLabel : markValue(row)} onWheel={preventNumberInputWheel} onChange={(event) => setResultField(row, 'marca', isTimePhase ? formatTimeEntryInput(event.target.value) : event.target.value)} placeholder={rowDns ? 'DNS' : rowDnf ? 'DNF' : isTimePhase ? (timeCapLabel || '12:00') : 'Valor'} disabled={rowDns || rowDnf || capDnf} />
                        </span>
                      </label>
                    ) : (
                      <span className="fr-result-readonly" data-label={markLabel} style={{ color: rowDns || rowDnf ? colors.error : colors.text, fontSize: 13, fontWeight: 850 }}>{displayMarkWithExtra(row)}</span>
                    )}
                    {showExtraInput ? (
                      <label className="fr-result-input-wrap">
