...
def render_judge_invitation(
    *,
    ...
    subject = f"Invitacion como juez - {competition_name}"
    text = (
        f"Hola {nombre},\n\n"
        f"{invited_by_name} te invito a ser juez en la competencia \"{competition_name}\".\n\n"
        "Para aceptar o rechazar la invitacion ingresa a FinalRep:\n"
        f"{invitation_url}\n\n"
        "Equipo FinalRep"
    )
    html = _html("Invitacion como juez", f"""\
      <p>Hola <strong>{nombre}</strong>,</p>
      <p><strong>{invited_by_name}</strong> te invito a participar como <strong>juez</strong> en esta competencia.</p>
      <hr class="divider">
      <p class="detail"><span>Competencia:</span> <strong>{competition_name}</strong></p>
      <hr class="divider">
      <p>Ingresa a FinalRep para aceptar o rechazar la invitacion:</p>
...
def render_profile_completion_reminder(
    *,
    nombre: str,
    missing_fields: list[str],
    profile_url: str = "https://finalrep.co/profile",
    is_enrolled: bool = False,
) -> tuple[str, str, str]:
    labels = [_FIELD_LABELS.get(f, f) for f in missing_fields]
    fields_text = "\n".join(f"  - {label}" for label in labels)
    fields_html = "".join(f'<li style="color:#F5F7FA;line-height:1.8">{label}</li>' for label in labels)
    reason = (
        "Tienes una inscripcion activa. Completa tu perfil para mantener tu participacion y recibir informacion del evento sin inconvenientes."
        if is_enrolled
        else "Tu perfil aun esta incompleto. Completarlo te permite participar en competencias y recibir invitaciones sin inconvenientes."
    )

    subject = "Completa tu perfil en FinalRep"
    text = (
        f"Hola {nombre},\n\n"
        f"{reason}\n\n"
        f"Campos pendientes:\n{fields_text}\n\n"
        f"Completa tu perfil aqui: {profile_url}\n\n"
        "Equipo FinalRep"
    )
    html = _html("Completa tu perfil", f"""\
      <p>Hola <strong>{nombre}</strong>,</p>
      <p>{reason}</p>
      <hr class="divider">
      <p style="color:#AAB2C0;margin-bottom:6px">Campos pendientes:</p>
      <ul style="margin:0;padding-left:20px">{fields_html}</ul>
      <hr class="divider">
      <a href="{profile_url}" class="btn">Completar mi perfil</a>
      <p class="muted">Si ya completaste tu perfil, puedes ignorar este correo.</p>
    """)
    return subject, text, html
